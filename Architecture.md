...
- **Host:** Vercel (per `vercel.json` — a plain SPA rewrite of every path to `/index.html`, plus `Cache-Control: public, max-age=31536000, immutable` on `/assets/*`). No other infrastructure (no edge functions, no serverless functions, no custom headers beyond the cache rule) is configured in this repository.
