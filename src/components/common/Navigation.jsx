...
// Custom hook for body scroll management
...
// Reusable navigation item components
const DesktopNavItem = ({ item, isActive, userRole, onNavigate }) => {
  const Icon = item.icon;
  const isAccessible = typeof item.accessible === 'function' 
    ? (userRole ? item.accessible(userRole) : true)
    : item.accessible;
...
const MobileSidebarItem = ({ item, isActive, userRole, onNavigate }) => {
  const Icon = item.icon;
  const isAccessible = typeof item.accessible === 'function'
    ? (userRole ? item.accessible(userRole) : true)
    : item.accessible;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 text-left min-w-0">
        ...
        <p className={`text-xs mt-0.5 text-gray-500 line-clamp-2 ${isActive ? 'text-blue-700' : 'text-gray-500'}`}>
          {item.description}
        </p>
      </div>
...
const MobileBottomNavItem = ({ item, isActive, userRole, onNavigate }) => {
  const Icon = item.icon;
  const isAccessible = typeof item.accessible === 'function'
    ? (userRole ? item.accessible(userRole) : true)
    : item.accessible;
  ...
  return (
    <button
      ...
      className={`
        ...
        active:scale-95 touch-manipulation min-w-0 px-1
        ${!isAccessible 
          ? 'opacity-50 cursor-not-allowed'
          : isActive
          ? 'text-blue-600'
    ...
    >
      ...
      {isActive && (
        <div className="absolute -top-0.5 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-blue-600 rounded-b-full" />
      )}
    </button>
  );
};
...
const DesktopNavigation = ({ navigationItems, currentPage, userRole, onNavigate }) => (
  <nav className="hidden lg:block bg-white border-b border-gray-200 shadow-sm sticky top-0 z-30">
...
// Mobile sidebar component with improved scrollable area
const MobileSidebar = ({ isOpen, onClose, navigationItems, currentPage, userRole, onNavigate }) => (
  <aside
    className={`
...
const MobileBottomNavigation = ({ navigationItems, currentPage, userRole, onNavigate }) => (
  <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-30 safe-bottom">
...
function Navigation({ currentPage, onNavigate, userRole, isOpen, onClose }) {
  ...
  const navigationItems = useNavigationItems(userRole);
